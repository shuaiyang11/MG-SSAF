...
class Attention(nn.Module):
    def __init__(self, dim, input_resolution, ratio, width, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.):
        ...
        local_heads = num_heads if self.ratio == 0 else int(self.num_heads * (1 - self.ratio))
    ...
    def forward(self, x):
        ...
        if not self.ratio:
            ...
            l_num_heads = int(self.num_heads * (1 - self.ratio))
            g_num_heads = int(self.num_heads * self.ratio)

            l_dim = int(2 * C * (1 - self.ratio))
            l_feats = qkv[:, 0:l_dim, :, :]
            g_feats = qkv[:, l_dim:, :, :]
